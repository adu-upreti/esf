...
import { X, Calendar, MapPin, Users, Expand } from "lucide-react";
...
export function OurWorkSection() {
  ...
  // Function to handle flash card click
  const handleFlashCardClick = (projectId: number) => {
    // Save scroll position only when flash card is clicked
    const currentScrollY = window.scrollY;
    setSavedScrollPosition(currentScrollY);
    console.log("Flash card clicked, saved scroll position:", currentScrollY);
  ...
  const handleCrossClick = () => {
    console.log(
      "Cross button clicked, restoring scroll position:",
      savedScrollPosition
    );
  ...
  const projects = [
    ...
    },
    {
      id: 2,
      title:
        "GHG and Carbon Footprint Assessment – Purnima Foods Industries Pvt. Ltd.",
      year: "2025",
          ...
          {displayProjects.map((project, index) => (
            ...
            >
              <Card
                ...
                onClick={() => handleFlashCardClick(project.id)}
              >
                  ...
                  <div>
                    <div className="relative h-64">
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        className="absolute top-4 right-4 bg-esf-primary/30 hover:bg-esf-primary/50 backdrop-blur-sm border border-esf-primary/20"
                        onClick={handleCrossClick}
                      >
                        <X className="w-6 h-6 text-esf-primary" />
                      </Button>
                      <Button
                        variant="ghost"
                        size="icon"
                        className={`absolute top-4 right-16 backdrop-blur-sm border transition-all duration-300 ${
                          expandClicked
                            ? "bg-green-500/50 hover:bg-green-500/70 border-green-500/50"
                            : "bg-esf-primary/30 hover:bg-esf-primary/50 border-esf-primary/20"
                        }`}
                        onClick={() => {
                          const imageUrl = projects.find(
                          ...
                          setTimeout(() => setExpandClicked(false), 2000);
                        }}
                      >
                        <Expand
