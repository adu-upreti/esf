...
export function ContactSection() {
  ...
  // Web3Forms access keys - replace with your actual keys from web3forms.com
  const WEB3FORMS_INQUIRY_ACCESS_KEY =
    process.env.NEXT_PUBLIC_WEB3FORMS_INQUIRY_ACCESS_KEY ||
    "YOUR_INQUIRY_ACCESS_KEY_HERE";

  const WEB3FORMS_QUOTATION_ACCESS_KEY =
    process.env.NEXT_PUBLIC_WEB3FORMS_QUOTATION_ACCESS_KEY ||
    "YOUR_QUOTATION_ACCESS_KEY_HERE";
  ...
  const handleInquirySubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setInquiryStatus({ status: "loading", message: "" });

    // Check if access key is configured
    if (WEB3FORMS_INQUIRY_ACCESS_KEY === "YOUR_INQUIRY_ACCESS_KEY_HERE") {
      setInquiryStatus({
        status: "error",
        message:
          "Web3Forms access key not configured. Please contact the administrator.",
      });
      return;
    }
    ...
    try {
      const response = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          access_key: WEB3FORMS_INQUIRY_ACCESS_KEY,
          name: inquiryData.name,
          email: inquiryData.email,
          subject: `General Inquiry: ${inquiryData.subject}`,
          message: inquiryData.message,
          from_name: "ESF Website Contact Form",
          reply_to: inquiryData.email,
          // Send to the general inquiry email
          to: "info@everestsustainability.com",
        }),
      });
  ...
  const handleQuotationSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setQuotationStatus({ status: "loading", message: "" });

    // Check if access key is configured
    if (WEB3FORMS_QUOTATION_ACCESS_KEY === "YOUR_QUOTATION_ACCESS_KEY_HERE") {
      setQuotationStatus({
        status: "error",
        message:
          "Web3Forms access key not configured. Please contact the administrator.",
    ...
    try {
      const response = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          access_key: WEB3FORMS_QUOTATION_ACCESS_KEY,
          name: quotationData.companyName,
          email: quotationData.email,
          subject: `Quotation Request: ${quotationData.subject}`,
          message: `Company: ${quotationData.companyName}\n\n${quotationData.message}`,
          from_name: "ESF Website Quotation Form",
          reply_to: quotationData.email,
          // Send to the quotation email
          to: "nishchalbaniya@everestsustainability.com",
        }),
      });
      ...
      if (result.success) {
        setQuotationStatus({
          status: "success",
          message:
            "Thank you for your quotation request! We will get back to you soon.",
        });
        setQuotationData({
          companyName: "",
          email: "",
            ...
            <Card className="p-8">
              ...
              <form onSubmit={handleInquirySubmit} className="space-y-6">
                ...
                <Button
                  ...
                  size="lg"
                  disabled={inquiryStatus.status === "loading"}
                >
                  {inquiryStatus.status === "loading"
                    ? "Sending..."
                    : t("contact.submit")}
                </Button>

                {/* Status Message */}
                {inquiryStatus.status !== "idle" && (
                  <div
                  ...
                  >
                    ...
                    <span className="text-sm">{inquiryStatus.message}</span>
                  </div>
                )}
              </form>
            </Card>
            ...
            <Card className="p-8">
              ...
              <form onSubmit={handleQuotationSubmit} className="space-y-6">
                ...
                <Button
                  ...
                  className="w-full dark:border-[#fdfdfd]"
                  size="lg"
                  disabled={quotationStatus.status === "loading"}
                >
                  {quotationStatus.status === "loading"
                    ? "Sending..."
                    : t("contact.requestQuotation")}
                </Button>

                {/* Status Message */}
                {quotationStatus.status !== "idle" && (
                  <div
                    className={`flex items-center gap-2 p-3 rounded-lg ${
                      quotationStatus.status === "success"
                        ? "bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400"
                        : "bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400"
                    }`}
                  >
                    ...
                    <span className="text-sm">{quotationStatus.message}</span>
                  </div>
                )}
              </form>
            </Card>
